"""
dashboard_gen.py
3-tab HTML dashboard: Overview | View Log | Equity Curve
Equity chart deferred to tab-click to avoid canvas-in-hidden-div bug.
"""
...
def _fmt_time(iso):
    ...
    except: return iso or "&mdash;"

def _view_badge(view):
    col = VIEW_COLOR.get(view, "#888")
    return (f"<span style='background:{col}20;color:{col};"
            f"border:1px solid {col}60;padding:2px 10px;"
            f"border-radius:12px;font-size:10px;font-weight:700;"
            f"text-transform:uppercase;letter-spacing:1px'>{view or '&mdash;'}</span>")
...
def _straddle_rows(snapshot, atm):
    ...
    for strike in sorted(snapshot.keys(), key=lambda x: int(x), reverse=True):
        ...
        rows  += (f"<tr style='{bg}'><td>{label}</td>"
                  f"<td>{f'{price:.1f}' if price else '&mdash;'}</td>"
                  f"<td>{f'{vwap:.1f}' if vwap else '&mdash;'}</td>"
                  f"<td style='color:{rc};font-weight:700'>{rel}</td></tr>")
    return rows
...
def _position_rows(positions):
    ...
    for p in positions:
        ...
        rows += (f"<tr><td>{st}</td>"
                 f"<td style='color:{tc};font-weight:700'>{p.get('opt_type','')}</td>"
                 f"<td>{p.get('strike','')}</td>"
                 f"<td style='color:{ac};font-weight:700'>{p.get('action','')}</td>"
                 f"<td>{p.get('lots','')}</td>"
                 f"<td>&#8377;{p.get('entry_price',0):.1f}</td>"
                 f"<td>&#8377;{p.get('current_ltp',0):.1f}</td>"
                 f"<td style='color:{pc};font-weight:700'>&#8377;{pnl:,.0f}</td></tr>")
    return rows
...
def _error_rows(errors):
    ...
    for e in reversed(errors[-10:]):
        rows += (f"<tr><td style='white-space:nowrap;color:var(--muted)'>{_fmt_time(e.get('time'))}</td>"
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .view-pill { display:inline-block; padding:4px 14px; border-radius:20px; font-weight:700; font-size:11px;
...
<div class="header">
  <h1>&#9889; Nifty Options</h1>
  <div class="header-meta"><span class="pulse"></span>PAPER TRADE &nbsp;&middot;&nbsp; ATM {atm or '&mdash;'} &nbsp;&middot;&nbsp; Last run: {last_run} &nbsp;&middot;&nbsp; Auto-refresh 5min</div>
</div>
<div class="tab-bar">
...
<div id="panel-overview" class="panel active">
  ...
  <div class="section">
    <div class="section-hdr"><h2>&#128202; Straddle VWAP</h2><span style="color:var(--muted);font-size:10px">ATM &#177;400 &middot; 9 strikes &middot; 5-min</span></div>
    <table><thead><tr><th>Strike</th><th>Straddle Price</th><th>VWAP</th><th>vs VWAP</th></tr></thead>
    <tbody>{straddle_html}</tbody></table>
  </div>
  <div class="section">
    <div class="section-hdr"><h2>&#128203; Positions</h2><span style="color:var(--muted);font-size:10px">Paper Trade</span></div>
    <table><thead><tr><th>St</th><th>Type</th><th>Strike</th><th>Action</th><th>Lots</th><th>Entry &#8377;</th><th>LTP &#8377;</th><th>P&amp;L</th></tr></thead>
    <tbody>{pos_html}</tbody></table>
  </div>
  <div class="section">
    <div class="section-hdr"><h2>&#128308; Error Log</h2><span style="color:var(--muted);font-size:10px">Last 10</span></div>
...
<div id="panel-equity" class="panel">
  <div id="eq-nodata" class="no-data" style="display:none">No equity data yet &mdash; populates after 10:30 AM entry</div>
...
<footer><span class="pulse"></span>Nifty Options Paper Trader &middot; {state.get('date','&mdash;')} &middot; &copy; {datetime.now().year}</footer>

<script>
const eqTimes = """ + eq_times_json + """;
const eqPnls  = """ + eq_pnls_json  + """;
